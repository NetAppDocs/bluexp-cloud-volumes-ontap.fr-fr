...
| \https://keyvault-production-aks.vault.azure.net | Coffre-fort de clés | Utilisé pour récupérer la clé secrète du client à partir du coffre-fort de clés Azure afin de communiquer avec le compartiment S3 pour le traitement des métadonnées. Le service Cloud Volumes ONTAP utilise ce composant en interne. | Modes standard, restreint et privé. | Les services Cloud Volumes ONTAP ne sont pas disponibles. 
