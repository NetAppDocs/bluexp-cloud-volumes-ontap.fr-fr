...
BlueXP alloue automatiquement le nombre requis d'adresses IP privées à Cloud Volumes ONTAP dans GCP. Vous devez vous assurer que votre réseau dispose de suffisamment d'adresses privées.

Le nombre de LIF attribué par Cloud Manager pour Cloud Volumes ONTAP dépend du déploiement d'un système à un seul nœud ou d'une paire haute disponibilité. Une LIF est une adresse IP associée à un port physique. Une LIF de gestion SVM est nécessaire pour les outils de gestion tels que SnapCenter.
...
* *Paire HA* BlueXP alloue 14 adresses IP à une paire HA :
+
...
TIP: Vous recherchez des informations sur le connecteur ? https://docs.netapp.com/us-en/cloud-manager-setup-admin/reference-ports-gcp.html["Afficher les règles de pare-feu du connecteur"^]
...
* https://docs.netapp.com/us-en/cloud-manager-setup-admin/task-creating-connectors-gcp.html["Afficher les exigences de mise en réseau du connecteur"^]
