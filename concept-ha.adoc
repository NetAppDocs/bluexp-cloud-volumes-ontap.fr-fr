...
Cloud Volumes ONTAP prend uniquement en charge les déploiements haute disponibilité dans les zones locales AWS. Voici des exemples de configurations :
...
Cloud Volumes ONTAP prend uniquement en charge ces configurations. La sélection d'un autre type de disque ou d'une instance EC2 dans la configuration AWS local zone pour Cloud Volumes ONTAP peut entraîner une panne de déploiement. Consultez la documentation AWS pour obtenir les informations les plus récentes et complètes sur le link:https://aws.amazon.com/about-aws/global-infrastructure/localzones/features/?nc=sn&loc=2["Types d'instances EC2 dans les zones locales"^].
