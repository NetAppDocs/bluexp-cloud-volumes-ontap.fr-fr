...
Notez que les limites de disques peuvent vous empêcher d'atteindre la limite de capacité en utilisant des disques seuls. Vous pouvez aller au-delà de la limite des disques de link:concept-data-tiering.html["tiering des données inactives vers le stockage objet"]. Pour plus d'informations sur les limites de disques, reportez-vous à la section https://["Limites de stockage dans les notes de mise à jour de Cloud Volumes ONTAP"^].
...
https://["Découvrez comment ajouter des comptes au site de support NetApp à BlueXP"^].
