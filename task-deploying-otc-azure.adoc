...
+
À partir de Cloud Volumes ONTAP 9.15.1, vous pouvez déployer des instances de VM dans des zones de disponibilité uniques de stockage redondant localement (LRS). Cette `Microsoft.Compute/VMOrchestratorZonalMultiFD` fonction doit également être activée pour votre abonnement. Cette fonctionnalité facilite le déploiement de paires haute disponibilité dans la même zone de disponibilité. Reportez-vous à link:task-saz-feature.html["Activez VMOrchestratorZonalMultiFD pour les zones de disponibilité uniques"]et link:task-azure-high-availability-mode.html["Activez le mode haute disponibilité dans Azure"].
...
+
*** Pour les zones de disponibilité uniques, sélectionnez une région Azure, une zone de disponibilité, un vnet et un sous-réseau. À partir de Cloud Volumes ONTAP 9.15.1, vous pouvez installer des instances de VM dans une zone de disponibilité unique. Vous devez sélectionner une région prenant en charge ce mode. Si la région sélectionnée par vous ne prend pas en charge le déploiement zonal, le modèle de déploiement non zonal précédent pour LRS est suivi.
+
Pour en savoir plus sur ce mode de déploiement et les zones Azure prises en charge, reportez-vous aux sections https://learn.microsoft.com/en-us/azure/virtual-machine-scale-sets/["Documentation Microsoft Azure : Virtual machine Scale Sets"^] et https://azure.microsoft.com/en-us/explore/global-infrastructure/products-by-region/["Site Web Microsoft Azure : produits disponibles par région"^].
...
. *Profil d'utilisation, type de disque et règle de hiérarchisation* : choisissez si vous souhaitez activer les fonctionnalités d'efficacité du stockage et modifiez la règle de hiérarchisation du volume, si nécessaire.
+
Pour plus d'informations, reportez-vous à link:https://docs.netapp.com/us-en/bluexp-cloud-volumes-ontap/task-planning-your-config-azure.html#choose-a-volume-usage-profile["Choisissez un profil d'utilisation du volume"^] et link:concept-data-tiering.html["Vue d'ensemble du hiérarchisation des données"^].
