...
Pour en savoir plus sur les volumes FlexClone, consultez le http://["Guide de gestion du stockage logique ONTAP 9"^].
...
+
image:screenshot_aggr_volume_overview.png["Capture d'écran : affiche la liste des volumes d'un agrégat dans la boîte de dialogue Informations sur l'agrégat."]
...
+
Pour plus de détails, voir link:https://docs.netapp.com/us-en/cloud-manager-replication/task-replicating-data.html["Réplication des données entre les systèmes"^].
...
+
Pour plus de détails, voir link:https://docs.netapp.com/us-en/cloud-manager-replication/task-replicating-data.html#managing-data-replication-schedules-and-relationships["Gestion des planifications et des relations de réplication des données"^].
...
+
image:screenshot_aggr_capacity.png["Capture d'écran : affiche la capacité totale d'agrégat et la capacité d'agrégat utilisée disponibles dans la boîte de dialogue Informations d'agrégat."]
...
image:screenshot_show_flexgroup_vol.png["Capture d'écran affichant l'icône de volume FlexGroup, placez le curseur de la souris sous la mosaïque volumes."]
