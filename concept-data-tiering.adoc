...
Les coûts d'accès sont plus élevés si vous accédez aux données. Prenez donc ces considérations avant de changer la classe de stockage. https://["En savoir plus sur les classes de stockage Amazon S3"^].
...
Les coûts d'accès sont plus élevés si vous accédez aux données, prenez donc ces considérations avant de changer le Tier de stockage. https://["En savoir plus sur les tiers d'accès au stockage Azure Blob"^].
...
+
--
Les coûts d'accès sont plus élevés si vous accédez aux données. Prenez donc ces considérations avant de changer la classe de stockage. https://["En savoir plus sur les classes de stockage pour Google Cloud Storage"^].
...
TIP: Vous pouvez modifier la période de refroidissement minimale et le seuil global par défaut de 50 % (plus d'informations sur ce point ci-dessous). http://["Découvrez comment changer la période de refroidissement"^] et http://["découvrez comment modifier le seuil"^].
