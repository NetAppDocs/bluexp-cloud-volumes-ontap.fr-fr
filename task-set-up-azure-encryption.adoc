...
Si vous créez une nouvelle version de votre clé, Cloud Volumes ONTAP utilise automatiquement la dernière version de la clé, garantissant ainsi la sécurité de vos données sans nécessiter de mises à jour manuelles.

.Étapes
. Accédez à votre coffre-fort de clés dans le portail Azure.
. Sélectionnez la touche que vous souhaitez faire pivoter.
. Cliquez sur *Nouvelle version* pour générer une nouvelle version de la clé.


Une fois qu'une nouvelle version est créée, Cloud Volumes ONTAP détecte et utilise automatiquement la dernière version de la clé pour le chiffrement.
