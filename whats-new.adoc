...
Les améliorations décrites sur cette page sont spécifiques aux fonctions BlueXP qui permettent la gestion de Cloud Volumes ONTAP. Pour découvrir les nouveautés du logiciel Cloud Volumes ONTAP lui-même, https://["Accédez aux notes de version de Cloud Volumes ONTAP"^]
...
https://["Découvrez comment afficher la capacité consommée dans votre compte"^].
...
https://["Découvrez les nouvelles fonctionnalités de cette version d'Cloud Volumes ONTAP"^].
...
En savoir plus sur https://["Tailles de disques prises en charge dans Azure"^].
...
https://["Découvrez comment chiffrer les volumes à l'aide des solutions de cryptage NetApp"^].
...
Pour plus d'informations sur la configuration du paramètre de création du groupe de placement, reportez-vous à la section link:https://docs.netapp.com/us-en/cloud-manager-cloud-volumes-ontap/task-configure-placement-group-failure-aws.html#overview["Configurez la création de groupe de placement pour AWS HA Single AZ"^].
...
link:https://docs.netapp.com/us-en/cloud-manager-cloud-volumes-ontap/task-enabling-private-link.html#provide-bluexp-with-details-about-your-azure-private-dns["Fournissez BlueXP avec des informations détaillées sur votre DNS privé Azure"^]
...
link:https://docs.netapp.com/us-en/cloud-manager-cloud-volumes-ontap/concept-worm.html["En savoir plus sur le stockage WORM"^]
...
https://["Découvrez comment configurer et gérer les services de stockage objet S3 dans ONTAP"^]
...
Pour plus d'informations sur la configuration et l'utilisation de NDMP, reportez-vous à la section https://["Présentation de la configuration NDMP"].
...
Pour plus d'informations sur cette nouvelle fonctionnalité, voir https://["Configuration de Cloud Volumes ONTAP pour utiliser une clé gérée par le client dans Azure"].
...
+
https://["Apprenez à changer les méthodes de charge"].
...
* *Edge cache* : licences pour https://["Cloud volumes Edge cache"^]


https://["En savoir plus sur ces packs de licences"].
...
https://["En savoir plus sur la prise en charge d'Elastic volumes dans AWS"].
...
https://["Consultez la liste complète des régions prises en charge par Cloud Volumes ONTAP"^]
...
https://["En savoir plus sur la vue avancée"].
...
https://["En savoir plus sur la prise en charge d'Elastic volumes"].
...
"ec2:ModifyVolume",
----
Veillez à fournir ces autorisations à chaque ensemble d'identifiants AWS que vous avez ajoutés à BlueXP. https://["Consultez les dernières règles de connexion pour AWS"^].
...
https://["En savoir plus sur les connexions Azure Private Link avec Cloud Volumes ONTAP"].
...
* https://["En savoir plus sur les limites des machines virtuelles de stockage dans Google Cloud"^]
* https://["Découvrez comment créer des machines virtuelles de stockage destinées aux données pour Cloud Volumes ONTAP dans Google Cloud"]
...
https://["En savoir plus sur la gestion des données Kubernetes dans BlueXP"^]
...
https://["En savoir plus sur les licences"].
...
https://["Découvrez comment changer la classe de stockage des données hiérarchisées"].
...
https://["Consultez les configurations prises en charge pour Cloud Volumes ONTAP dans Google Cloud"^]
...
Vous pouvez toujours vous connecter à System Manager en entrant l'adresse IP de gestion du cluster dans un navigateur Web qui dispose d'une connexion au système Cloud Volumes ONTAP. https://["En savoir plus sur la connexion à System Manager"].
...
https://["Découvrez comment créer des volumes dans un agrégat spécifique"].
...
https://["En savoir plus sur les options de licence pour Cloud Volumes ONTAP"].
...
https://["Découvrez comment échanger des licences de nœuds non attribuées"].
...
Accédez au https://["Notes de version de Cloud Volumes ONTAP"^] pour plus d'informations sur les configurations prises en charge,
...
* https://["En savoir plus sur les abonnements NetApp Keystone"^].
...
Les données sont automatiquement chiffrées sur Cloud Volumes ONTAP dans Azure à l'aide de https://["Chiffrement de service de stockage Azure"^] Et elle est dotée d'une clé gérée par Microsoft. Mais vous pouvez désormais utiliser votre propre clé de chiffrement gérée par le client en procédant comme suit :
...
https://["Affichez les configurations prises en charge pour Cloud Volumes ONTAP dans AWS"^].
...
----
Veillez à fournir ces autorisations à chaque ensemble d'identifiants Azure que vous avez ajoutés à BlueXP. https://["Découvrez la dernière règle de connecteurs pour Azure"^].
...
https://["Affichez les configurations prises en charge pour Cloud Volumes ONTAP dans GCP"^].
...
Accédez au https://["Page AWS Marketplace"^] pour consulter les détails des prix et consulter le https://["Notes de version de Cloud Volumes ONTAP"^] pour en savoir plus sur cette option de licence.
...
Notez qu'un connecteur requiert une nouvelle autorisation pour supprimer les snapshots Azure. https://["Découvrez la dernière règle de connecteurs pour Azure"^].
...
Cloud Volumes ONTAP peut maintenant utiliser un disque SSD Ultra en tant que VNVRAM lorsque vous utilisez le type de machine virtuelle E32S_v3 avec un système à un seul nœud https://["Dans toutes les régions Azure prises en charge"^].
...
* https://["En savoir plus sur les liens privés Azure"^]
* link:task-enabling-private-link.html["En savoir plus sur l'utilisation d'un lien privé Azure avec Cloud Volumes ONTAP"]
