...
Le connecteur doit être déployé dans la même région Azure que les systèmes Cloud Volumes ONTAP qu'il gère ou dans https://["Paire de régions Azure"^] Pour les systèmes Cloud Volumes ONTAP.
...
https://["Découvrez comment activer le trafic API direct sur le connecteur"^]
...
Si vous utilisez https://["DNS privé Azure"^], Vous devez ensuite modifier un fichier de configuration sur chaque connecteur. Sinon, BlueXP ne peut pas activer la connexion Azure Private Link entre Cloud Volumes ONTAP et les comptes de stockage associés.

Notez que le nom DNS doit correspondre aux exigences de nommage des DNS Azure https://["Comme illustré dans la documentation Azure"^].
