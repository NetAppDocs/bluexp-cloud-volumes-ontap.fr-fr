...
** Compte de service associé au connecteur https://["doit disposer des autorisations requises"^]
...
* Les API Google Cloud doivent être de https://["activé dans votre projet"^]:
+
...
NOTE: Il s'agit du compte de service que vous avez configuré pour BlueXP, link:https://docs.netapp.com/us-en/cloud-manager-setup-admin/task-quick-start-connector-google.html["comme décrit sur cette page"^].
...
|===
+
La vidéo suivante explique comment associer un abonnement au Marketplace à paiement basé sur l'utilisation à votre projet Google Cloud. Vous pouvez également suivre les étapes pour vous abonner à la https://["Association d'un abonnement Marketplace aux identifiants Google Cloud"^] section.
...
| Vérification de la connectivité | Pour déplacer des données inactives vers un compartiment Google Cloud Storage, le sous-réseau dans lequel réside Cloud Volumes ONTAP doit être configuré pour un accès Google privé. Pour obtenir des instructions, reportez-vous à la section https://["Documentation Google Cloud : configuration de Private Google Access"^]. 
...
+
Pour obtenir de l'aide sur le choix du type et de la taille d'un disque, reportez-vous à la section link:task-planning-your-config-gcp.html#sizing-your-system-in-gcp["Dimensionnement du système dans Google Cloud"].
...
** Pour obtenir de l'aide sur l'ajout d'un compte Google Cloud, consultez la section https://["Configuration et ajout de comptes Google Cloud pour le Tiering des données avec 9.6"^].
...
| Nom du compte de service | Si vous avez l'intention d'utiliser le link:concept-data-tiering.html["Tiering BlueXP"] ou https://["Sauvegarde et restauration BlueXP"^] Services, vous devez activer le commutateur *compte de service*, puis sélectionner le compte de service qui a le rôle d'administrateur de stockage prédéfini. 
...
NOTE: _Flash cache_, une vitesse d'écriture élevée et une unité de transmission maximale (MTU) supérieure de 8,896 octets sont disponibles via l'option de vitesse d'écriture *élevée* avec les types d'instance n2-standard-16, n2-standard-32, n2-standard-48 et n2-standard-64. En outre, pour augmenter la MTU de 9 8,896, les VPC-1, VPC-2 et VPC-3 doivent être sélectionnés pour le déploiement. _Flash cache_, une vitesse d'écriture élevée et un MTU de 9 8,896 dépendent des fonctionnalités et ne peuvent pas être désactivés individuellement dans une instance configurée. Pour plus d'informations sur les modèles VPC-1, VPC-2 et VPC-3, reportez-vous à la section https://["Règles pour VPC-1, VPC-2 et VPC-3"].
