...
NOTE: Il s'agit du compte de service que vous avez configuré pour BlueXP, link:https://docs.netapp.com/us-en/cloud-manager-setup-admin/task-quick-start-connector-google.html#set-up-a-service-account-for-the-connector["comme décrit sur cette page"^].
...
TIP: Si vous souhaitez utiliser le stockage WORM et le Tiering des données, vous devez désactiver Cloud Backup et déployer un environnement de travail Cloud Volumes ONTAP avec la version 9.8 ou supérieure.
